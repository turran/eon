...
#include "eon_main.h"
#include "eon_element_splitter.h"

#include "eon_renderable_private.h"
#include "eon_layout_splitter_private.h"
#include "eon_orientation_private.h"

/* A splitter must "split" all its children, for that we need:
 * Make the layout stack support external callback for each child
 * Create a split widget after the first element inserted?
 */ 
/*============================================================================*
 *                                  Local                                     *
 *============================================================================*/
#define EON_ELEMENT_SPLITTER_DESCRIPTOR eon_element_splitter_descriptor_get()
#define EON_ELEMENT_SPLITTER(o) ENESIM_OBJECT_INSTANCE_CHECK(o,			\
		Eon_Element_Splitter, EON_ELEMENT_SPLITTER_DESCRIPTOR)

typedef struct _Eon_Element_Splitter
{
	Eon_Renderable base;
	/* attributes */
...
} Eon_Element_Splitter;

typedef struct _Eon_Element_Splitter_Class
{
	Eon_Renderable_Class base;
} Eon_Element_Splitter_Class;

static Egueb_Dom_Node * _eon_element_splitter_event_get_renderable(
		Eon_Element_Splitter *thiz, Egueb_Dom_Event *e)
{
...
static void _eon_element_splitter_node_inserted_cb(Egueb_Dom_Event *e,
		void *data)
{
	Eon_Element_Splitter *thiz = data;
	Egueb_Dom_Node *target;
	Egueb_Dom_Node *attr;

	target = _eon_element_splitter_event_get_renderable(thiz, e);
	if (!target) return;
...
static void _eon_element_splitter_node_removed_cb(Egueb_Dom_Event *e,
		void *data)
{
	Eon_Element_Splitter *thiz = data;
	Egueb_Dom_Node *target;
...
static int _eon_element_splitter_size_hints_get(Eon_Renderable *r,
		Eon_Renderable_Size *size)
{
	...
	if (orientation == EON_ORIENTATION_HORIZONTAL)
		return eon_layout_splitter_horizontal_size_hints_get(n, size,
				&thiz->min_length, &thiz->weights);
	else
		return eon_layout_splitter_vertical_size_hints_get(n, size,
				&thiz->min_length, &thiz->weights);
}
...
static void _eon_element_splitter_init(Eon_Renderable *r)
{
	Eon_Element_Splitter *thiz;
	Egueb_Dom_Node *n;
	...
	egueb_dom_node_event_listener_add(n,
			...
			_eon_element_splitter_node_inserted_cb,
			EINA_FALSE, r);
	egueb_dom_node_event_listener_add(n,
			EGUEB_DOM_EVENT_MUTATION_NODE_REMOVED,
			_eon_element_splitter_node_removed_cb,
			EINA_FALSE, r);
...
static Egueb_Dom_Node * _eon_element_splitter_element_at(Eon_Renderable *r,
 ...
 *                             Element interface                              *
 *----------------------------------------------------------------------------*/
static Egueb_Dom_String * _eon_element_splitter_tag_name_get(Eon_Element *e)
{
	return egueb_dom_string_ref(EON_ELEMENT_SPLITTER);
}

static Eina_Bool _eon_element_splitter_child_appendable(Eon_Element *e, Egueb_Dom_Node *child)
{
	Egueb_Dom_Node *n;
 ...
 *                              Object interface                              *
 *----------------------------------------------------------------------------*/
ENESIM_OBJECT_INSTANCE_BOILERPLATE(EON_RENDERABLE_DESCRIPTOR, Eon_Element_Splitter,
		Eon_Element_Splitter_Class, eon_element_splitter);

static void _eon_element_splitter_class_init(void *k)
{
	Eon_Element_Class *klass;
	Eon_Renderable_Class *r_klass;

	klass = EON_ELEMENT_CLASS(k);
	klass->tag_name_get = _eon_element_splitter_tag_name_get;
	klass->child_appendable = _eon_element_splitter_child_appendable;

	r_klass = EON_RENDERABLE_CLASS(k);
	r_klass->init = _eon_element_splitter_init;
	r_klass->renderer_get = _eon_element_splitter_renderer_get;
	r_klass->size_hints_get = _eon_element_splitter_size_hints_get;
	r_klass->process = _eon_element_splitter_process;
	r_klass->element_at = _eon_element_splitter_element_at;
}

static void _eon_element_splitter_instance_init(void *o)
{
	Eon_Element_Splitter *thiz = o;
 ...
 *                                   API                                      *
 *============================================================================*/
EAPI Egueb_Dom_Node * eon_element_splitter_new(void)
{
	Egueb_Dom_Node *n;
	n = EON_ELEMENT_NEW(eon_element_splitter);
	return n;
}
